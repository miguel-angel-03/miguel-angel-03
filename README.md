# Miguel A. Martinez
### Bilingual Operations & Data Analyst | Business Administrator

Business Administrator and Data Analyst focused on transforming complex datasets into actionable business strategies. My professional approach combines strategic management with technical expertise in data engineering and process automation to drive financial and operational results.

### Professional Profile
- Specialized in Market Intelligence and Business Analytics.
- Experience in Digital Transformation and Process Automation.
- Proficiency in managing end-to-end analytical pipelines using the CRISP-DM methodology.
- Languages: Spanish (Native) | English (Advanced).

### Technical Stack
- **Business Intelligence:** Tableau, Power BI, Looker Studio.
- **Data Engineering & Analysis:** Python (Pandas, Numpy), SQL (BigQuery, PostgreSQL).
- **Automation:** Python Automation Scripts, Advanced Excel (VBA, Power Query).
- **Methodologies:** CRISP-DM, Financial Modeling, Strategic Planning.

### Featured Projects
#### [Market-Sizing-Wellness-Analytics](https://github.com/miguel-angel-03/Market-Sizing-Wellness-Analytics.git)
Comprehensive study to quantify the market potential for a subscription-based business model in Colombia.
- **Tech Stack:** Python, Google BigQuery, Power BI.
- **Key Outcome:** Identified a high-precision SOM of 12,319 users with a projected $3.36B COP ARR.

#### [lendingclub-credit-risk-analysis](https://github.com/miguel-angel-03/lendingclub-credit-risk-analysis.git)
Data-driven analysis of credit risk and loan performance using financial datasets.
- **Tech Stack:** Python, SQL.
- **Focus:** Risk mitigation and portfolio performance metrics.

### Contact Information
- **LinkedIn:** [https://www.linkedin.com/in/miguel-martinez-699188352/]
- **Upwork:** [https://www.upwork.com/freelancers/~01c829bb2ba8c1b51b?mp_source=share]
- **Email:** [migue.m.v2011@gmail.com]

![Miguel's GitHub Stats](https://github-readme-stats.vercel.app/api?username=miguel-angel-03&show_icons=true&theme=vibrant)
